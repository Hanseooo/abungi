...
export function RouteScreen(){
  ...
  const [inventoryOpen,setInventoryOpen]=useState(false);const [selection,setSelection]=useState<FieldItemCommand|null>(null);const [discardingItemId,setDiscardingItemId]=useState<string|null>(null);const [pendingNodeId,setPendingNodeId]=useState<string|null>(null);const [abandoning,setAbandoning]=useState(false);const dialogRef=useRef<HTMLDialogElement>(null);
  ...
  return <main className="screen route-screen" style={{'--region-bg':`url(${regionBackgroundUrl(run.regionIndex)})`} as CSSProperties}><GameHeader title={`Region ${run.regionIndex+1}`} subtitle="Choose your path."/>
    ...
    <footer className="route-footer">{abandoning
      ? <section className="route-choice-confirm" aria-live="polite"><p><strong>ABANDON THIS RUN?</strong> The run ends now and cannot be resumed.</p><div><PaperButton variant="danger" disabled={isResolving} onClick={()=>void abandon()}>DISCARD THIS RUN</PaperButton><PaperButton variant="quiet" disabled={isResolving} onClick={()=>setAbandoning(false)}>KEEP PLAYING</PaperButton></div></section>
      : <PaperButton variant="danger" disabled={isResolving} onClick={()=>setAbandoning(true)}>ABANDON RUN</PaperButton>}</footer>
