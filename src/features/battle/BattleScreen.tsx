...
function UnitFigure({unit,move,onTarget,validTarget,pulse,events,effects,current,onInfo}:{unit:BattleUnit;move?:AbilityDefinition;onTarget?:(id:string)=>void;validTarget:boolean;pulse:number;events:CombatEvent[];effects:BattleEffectInstance[];current:boolean;onInfo:()=>void}){
  ...
  const content=<><div className={`unit-cutout-wrap ${statusClass(unit)} ${unit.guardActive?'has-guard':''} ${hit?'is-hit':''} ${healed?'is-healed':''} ${stamped?'is-status-stamp':''} ${guarding?'is-guarding':''} ${revived?'is-revived':''} ${ko||!unit.alive?'is-ko':''} ${acting?'is-acting':''}`} data-pulse={pulse}><div className="status-aura" aria-hidden="true"><i/><i/><i/></div><CutoutArt assetId={assetFor(unit)} name={unit.displayName}/>{hitEvents.slice(-3).map((damage,index)=><span key={`d-${pulse}-${index}`} style={{'--pop-index':index} as CSSProperties} className={`number-pop ${damage.critical?'critical':''} affinity-${damage.affinity}`}>-{damage.amount}{damage.critical?' CRIT':''}{damage.affinity==='advantage'?' · ADV!':damage.affinity==='resisted'?' · RESIST':''}</span>)}{healEvents.slice(-2).map((heal,index)=><span key={`h-${pulse}-${index}`} style={{'--pop-index':index} as CSSProperties} className="number-pop heal-pop">+{heal.amount}</span>)}</div><div className={`unit-label ${unit.alive?'':'is-down'}`}><div><strong>{unit.displayName}</strong><AffinityMark affinity={unit.affinity} small/></div><Meter value={unit.hp} max={unit.maxHp} label={`${unit.displayName} HP`}/><StatusStrip statuses={unit.statuses} effects={effects}/>{move&&unit.side==='enemy'&&validTarget&&<span className={`target-read target-${relationship(move,unit)}`}>{relationship(move,unit).toUpperCase()}</span>}{current&&<span className="turn-flag">ACTING</span>}{!unit.alive&&<span className="down-flag">DOWN</span>}</div></>;
...
function BattleVfx({action,events}:{action?:Extract<CombatEvent,{type:'actionStart'}>;events:CombatEvent[]}){
  ...
  return <div className={`battle-vfx-layer choreo-${action.choreography??'utility'} ${action.signature?'signature':''}`} aria-hidden="true" key={`${action.actorId}:${action.actionId}`}><i className="vfx-path"/><i className="vfx-impact"/><i className="vfx-particle p1"/><i className="vfx-particle p2"/><i className="vfx-particle p3"/>{multi>1&&<span className="vfx-multihit">×{multi}</span>}{summon&&<span className="vfx-summon-mark">ASSEMBLE</span>}</div>;
}
...
export function BattleScreen(){
  ...
  return <main className={`battle-screen speed-${settings.animationSpeed}x ${settings.reducedMotion?'reduced-motion':''} ${screenShake?'battle-shake':''}`} style={{'--battle-bg':`url(${regionBackgroundUrl(run.regionIndex)})`} as CSSProperties}>
    ...
    <section className="ally-stage" aria-label="Party">{battle.allies.map(id=>{const unit=battle.units[id];const valid=side==='ally'&&(selectedItem?itemTargetLegal(selectedItem,id).legal:unit.alive);return <UnitFigure key={id} unit={unit} effects={battle.effects.filter(effect=>effect.targetUnitId===unit.id)} move={selectedSkill??undefined} onTarget={side==='ally'?selectTarget:undefined} validTarget={valid} pulse={pulse} events={events} current={presentedActor?.id===id} onInfo={()=>openCharacter(unit.sourceId)}/>})}</section>
    <DeployableField battle={battle} events={events}/>
    <section className="battle-hud">{actor&&actor.side==='ally'?<><div className="actor-ticket" style={charAccentStyle(getCharacter(actor.sourceId))}><span>CURRENT</span><h2>{actor.displayName}</h2><AffinityMark affinity={actor.affinity}/><StatusStrip statuses={actor.statuses} effects={battle.effects.filter(effect=>effect.targetUnitId===actor.id)}/>{actor.sourceId==='yatords'&&<b className="mechanic-counter">MOMENTUM {Number(actor.flags.momentum??0)}/3</b>}{actor.sourceId==='hans'&&<b className="mechanic-counter">DEPLOYS {battle.deployables.filter(d=>d.ownerId===actor.id).length}/2</b>}</div></>:<div className="actor-ticket enemy-thinking"><span>ENEMY TURN</span><h2>Watch the table.</h2></div>}</section>
