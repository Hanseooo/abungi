...
import { useAppStore, currentActorAvailability, skillTargetLegality, inventoryItems } from '../../app/appStore';
...
export function BattleScreen(){
  ...
  useEffect(()=>{if(events.length)void audioEngine.combatSfx(events);},[events,director.index,pulse]);
  ...
  // The highlight has to run the same check the command will, or a unit lights up and then refuses.
  const skillTargetValid=(targetId:string):boolean=>
    !actor||actor.side!=='ally'?false:skillTargetLegality(battle,actor.id,selectedSkill!.id,[targetId]).legal;
  ...
  return <main className={`battle-screen speed-${settings.animationSpeed}x ${settings.reducedMotion?'reduced-motion':''} ${screenShake?'battle-shake':''} ${entrance?'encounter-entering':''} ${scenePending?'entrance-paused':''}`} style={{'--battle-bg':`url(${regionBackgroundUrl(run.regionIndex)})`,'--entrance-duration':`${entrance?.duration??2400}ms`} as CSSProperties}>
    ...
    <section className={`action-tray ${isResolving?'locked':''}`} aria-busy={isResolving}>
      ...
      {tray==='skills'&&<div className="skill-grid">{availability.map((entry,index)=>{const pp=actor?.abilityPP?.[entry.ability.id]??0;const selected=selectedSkill?.id===entry.ability.id;return <div className={`skill-slot ${selected?'selected':''}`} key={entry.ability.id}><button className="skill-main" disabled={isResolving||!entry.legal} aria-pressed={selected} onClick={()=>{setSelectedItem(null);setSelectedSkill(selected?null:entry.ability)}}><span className="keycap">{index+1}</span><strong>{entry.ability.name}{actor?.upgradedAbilities?.includes(entry.ability.id)?'+':''}</strong><AffinityMark affinity={entry.ability.affinity} small/><span className="pp-count">PP <b>{pp}</b>/{maxPp(entry.ability)}</span><small>{entry.ability.description}</small>{!entry.legal&&<em>{entry.reason}</em>}</button><button className="skill-info" onClick={()=>openMove(entry.ability.id)} aria-label={`Info about ${entry.ability.name}`}>i</button></div>})}</div>}
