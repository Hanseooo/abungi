...
export function RewardScreen(){
  const run=useAppStore(s=>s.run)!;const reward=run.pendingReward!;const claim=useAppStore(s=>s.claimRewardChoice);const openItem=useAppStore(s=>s.openItemInfo);
  ...
  return <main className="screen reward-screen"><GameHeader title={reward.tier==='boss'?'Boss Cleared':reward.tier==='elite'?'Elite Spoils':'Battle Reward'} subtitle={`${reward.coins} encounter-scaled coins are secured. ${reward.bossRecovery?'Boss recovery will also restore part of the party.':''}`}/>
    <section className="reward-ledger"><div><span>COINS</span><strong>+{reward.coins}</strong></div>{reward.itemId&&<button className="reward-ledger-item" onClick={()=>openItem(reward.itemId!)}><span>FOUND ITEM</span><strong>{getItem(reward.itemId).name}</strong><small>{getItem(reward.itemId).rarity.toUpperCase()} · INFO</small></button>}{reward.bossRecovery&&<div><span>RECOVERY</span><strong>HP + PP</strong></div>}</section>
    {reward.spoilsChoices.length>0&&<section className="choice-section spoils-section"><h2>Choose your spoils</h2><p className="choice-explainer">The fight already paid its base coins. This is the small strategic extra: greed, sustain, or occasional salvage.</p><div className="choice-grid">{reward.spoilsChoices.map(option=><button key={option.id} className={`reward-choice spoils-choice ${spoils===option.id?'selected':''}`} aria-pressed={spoils===option.id} onClick={()=>setSpoils(option.id)}><span className="spoils-stamp">{option.id.toUpperCase()}</span><strong>{option.label}</strong><p>{option.description}</p>{option.itemId&&<small>{getItem(option.itemId).name}</small>}</button>)}</div></section>}
