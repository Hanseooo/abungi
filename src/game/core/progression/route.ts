...
const STAGE_POOLS: readonly (readonly NodeType[])[] = [
  ...
  ['battle','event','elite'],       // Final optional pressure choice.
];
...
function makeNode(region:number, stage:number, lane:number, rng:SeededRng):RouteNode {
  ...
  if (type === 'event') node.eventId = rng.pick(EVENT_IDS);
  return node;
}
...
export function generateRegionRoute(regionIndex:number, rng:SeededRng):RegionRoute {
  ...
  for (let stage=0; stage<BALANCE.routeStagesBeforeBoss; stage+=1) {
    const nodes = Array.from({length:2}, (_, lane) => makeNode(regionIndex, stage, lane, rng));
