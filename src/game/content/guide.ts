...
export const EFFECT_GUIDE:EffectGuideEntry[]=[
  {id:'protect',name:'Protect',short:'PROT',positive:true,anchor:'Until the start of the protector’s second turn from now.',trigger:'The first hit that redirects a positive share of damage.',description:`The protected ally keeps half of the incoming damage. The protector takes half of that redirected half, reduced by up to ${BALANCE.saq.classMonitorPrevention} HP once per round by Class Monitor. The protector's share takes no second Guard, Fortified or relic reduction, and it can knock the protector out. Protect does not cleanse, does not stop HP costs, and does not redirect the attack's status effects.`},
  {id:'ink-mark',name:'Ink Mark',short:'INK',positive:false,anchor:'Until the start of Ken’s second turn from now.',trigger:'The next successful direct damaging skill hit against this enemy.',description:`Multiplies that one hit's damage by ${BALANCE.ken.inkMarkDamageMultiplier}, taking the consuming skill's affinity, then disappears. Another ally consuming it adds a further +${Math.round(BALANCE.ken.collaborativeWorkDamageBonus*100)}% once per round; Needlework adds +${Math.round(BALANCE.ken.needleworkMarkDamageBonus*100)}% to its own consuming hit. It never multiplies across the hits of a multi-hit move, creates no second attack, and is removed by cleansing after ordinary negatives.`},
  {id:'script',name:'Script',short:'SCRIPT',positive:true,anchor:'Until the start of Ken’s second turn from now.',trigger:`One hit landing for at least ${Math.round(BALANCE.ken.scriptThresholdPercent*100)}% of this ally's Max HP.`,description:`Prevents ${Math.round(BALANCE.ken.scriptPreventPercent*100)}% of that single large hit, at least ${BALANCE.ken.scriptPreventFloor} HP, then disappears. Repeated small hits never trigger it, and it cannot react to an HP cost or to a protector's transferred share. Unlike Fortified, which reduces every hit for its duration, Script is one conditional use.`},
  {id:'taunt',name:'Taunt',short:'TAUNT',positive:true,anchor:'Until the start of Marcus’s next turn.',trigger:'Every enemy attack that picks a single target.',description:'While a Taunt stands, enemy single-target attacks all pick the taunting ally, whatever their HP. Attacks that hit the whole party are unaffected, and the taunter still takes the hits normally: Guard, Fortified and relics all apply. It ends early if the taunter is knocked out.'},
];
